...
namespace ariel {
    ...
    bool operator!=(const NumberWithUnits &c1, const NumberWithUnits &c2) {
        double numeuals = NumberWithUnits::convert(c2.num, c2.s, c1.s);
        return numeuals != c1.num;
    }

    bool operator>=(const NumberWithUnits &c1, const NumberWithUnits &c2) {
        double numeuals = NumberWithUnits::convert(c2.num, c2.s, c1.s);
        return c1.num >= numeuals;
    ...
    istream &operator>>(istream &is, NumberWithUnits &c) {
        ...
        string str;
        char dummy = '\0';
        is >> num >> dummy >> str >> dummy;//Absorption of all values including profit
        if (!NumberWithUnits::checkUnit(str)) {
            throw std::invalid_argument(str + " is invalid unit");
        }
        c = NumberWithUnits(num, str);
        return is;
    }
}
